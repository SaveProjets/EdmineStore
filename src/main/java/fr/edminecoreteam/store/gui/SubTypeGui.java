...
public class SubTypeGui implements Listener
{
	...
	private static List<Integer> getArticleListBySubType(String type, String status) { return SectionSQL.getArticleListBySubType(type, status); }
	
	public static void gui(Player p, String type, int colorPane, int Page, int MaxPage) {
        ...
        if (settingInfo.getLang() == 0)
        {
            ...
            new BukkitRunnable() {
    	        ...
    	        public void run() {
    	        	...
    	        	ItemStack deco = new ItemStack(Material.STAINED_GLASS_PANE, 1, (short)colorPane);
                    ...
                    for (int articles : getArticleListBySubType(type, "enable")) {
                    	StoreInfo storeInfo = new StoreInfo(articles);
                    	int slot = storeInfo.getArticleItemSlot();
                    	if (storeInfo.getArticlePage() == Page)
                    	{
