...
public class StoreMainGui implements Listener {
    ...
    private static String getStoreStatus(String type) {
        return SectionSQL.getSectionStatus(type);
    }
        ...
        if (settingInfo.getLang() == 0) {
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    if (!p.getOpenInventory().getTitle().equalsIgnoreCase("§8Boutique")) {
                        cancel();
                    }
                    ++t;
                    if (t == 100) {
            ...
            }.runTaskTimer((Plugin) api, 40L, 15L);
